        ...
        with open(path, 'rt') as json_file: 
            ...
            for entry in json_files: 
                if entry["id"] == "GO:0000145":
                    ...
                    self.assertEqual(entry["def"], "\"A protein complex peripherally associated with the plasma membrane that determines where vesicles dock and fuse. At least eight complex components are conserved between yeast and mammals.\" [GOC:cilia, PMID:15292201, PMID:27243008, PMID:9700152]")
                ...
                if entry["id"] == "GO:0035567":
                    ...
                    self.assertEqual(entry["def"], "\"The series of molecular signals initiated by binding of a Wnt protein to a frizzled family receptor on the surface of the target cell, followed by propagation of the signal via effectors other than beta-catenin.\" [GOC:signaling]")
                    self.assertEqual(entry["comment"], "This term should only be used when Wnt receptor signaling occurs via a beta-catenin-independent route but the downstream effectors are unknown. If the downstream effectors are known, consider instead annotating to one of the children, or requesting a new term.")
                    self.assertRaises(KeyError, lambda: entry["subset"])
                    self.assertEqual(entry["synonym"], ["\"beta-catenin-independent Wnt receptor signaling pathway\" EXACT [GOC:signaling]", "\"non-canonical Wnt receptor signaling pathway\" EXACT []", "\"non-canonical Wnt receptor signalling pathway\" EXACT [GOC:mah]", "\"non-canonical Wnt-activated signaling pathway\" EXACT [GOC:signaling]"])
