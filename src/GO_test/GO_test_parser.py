...
def write_relationship_edges(relationship_edges):
    for edge in list_of_edges:
        if edge[2] != 'is_a':
            ...
            edge_dict['_from'] = "GO_test_term/" + edge[0]
            edge_dict['_to'] = "GO_test_term/" + edge[1]
        ...
        if 'intersection_of' in graph.node[curr_node]:
            for val in graph.node[curr_node]['intersection_of']:
                ...
                if len(intersection_term) == 1:
                    ...
                    edge_dict["_from"] = "GO_test_term/" + curr_node
                    edge_dict["_to"] = "GO_test_term/" + intersection_term[0]
                    edge_dict["intersection_type"] = ""
                else:
                    edge_dict['_key'] = str(curr_node + "__" + intersection_term[1] + "__" + intersection_term[0])
                    edge_dict["_from"] = "GO_test_term/" + curr_node
                    edge_dict["_to"] = "GO_test_term/" + intersection_term[1]
    ...
    for i in range(len(graph.node)):
        ...
        if 'disjoint_from' in graph.node[curr_node]:
            ...
            for val in graph.node[curr_node]['disjoint_from']:
                ...
                edge_dict["_from"] = "GO_test_term/" + curr_node
                edge_dict["_to"] = "GO_test_term/" + val
...
relationship_edges_path = '../utils/tmp/GO_test_edges_relationship.json'
isa_edges_path = '../utils/tmp/GO_test_edges_isa.json'
intersection_edges_path = '../utils/tmp/GO_test_edges_intersection_of.json'
disjoint_edges_path = '../utils/tmp/GO_test_edges_disjoint_from.json'
nodes_path = "../utils/tmp/GO_test_term.json"
